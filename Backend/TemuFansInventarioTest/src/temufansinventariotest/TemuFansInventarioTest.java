...
public class TemuFansInventarioTest {
    ...
    private static void test_listarAlmacen() {
        AlmacenBO almacenbo = new AlmacenBO();

        ArrayList<Almacen> lista = almacenbo.listarTodos();

        for (Almacen alm : lista) {
            System.out.println("IdAlmacen: " + alm.getIdAlmacen());
            System.out.println("Direccion: " + alm.getDireccion());
            System.out.println("-----------------");
        }
    }

    private static void test_ReporteAlmacen() {
        AlmacenBO almacenbo = new AlmacenBO();

        ArrayList<ConsultaStock> lista = almacenbo.reportePorId(1);

        for (ConsultaStock consulta : lista) {
            System.out.println("Codigo Producto: " + consulta.getProducto().getCodigo() + "-" + consulta.getProducto().getIdProducto());
            System.out.println("Nombre: " + consulta.getProducto().getNombre());
            System.out.println("Marca: " + consulta.getMarca().getNombre());
            System.out.println("nroTorre: " + consulta.getTorre().getNro_torre());
            System.out.println("Stock: " + consulta.getTorre().getCantidad_paquete());
